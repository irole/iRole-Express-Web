...
const uploadImage = multer({
    storage: multer.diskStorage({
        ...
        destination: (req, file, cb) => {
            // Get Type of File
            const type = path.parse(file.originalname).ext;
...
const getDirImage = (type) => {
    const videoTypes = [".mkv", ".mp4", ".mpeg", ".MKV", ".MP4", ".MPEG"];
    const audioTypes = [".MP3", ".mp3", ".FLAC", ".flac", ".WAV", ".wav"];
    const imageTypes = ['.png', '.PNG', '.jpg', '.JPG', '.jpeg', '.JPEG', '.svg', '.SVG'];
    const documentTypes = [".doc", ".DOC", ".docx", ".DOCX", ".ppt", ".PPT", ".pptx", ".PPTX", ".txt", ".TXT", ".pdf", ".PDF"];
    const compressTypes = [".rar", ".RAR", ".zip", ".ZIP", ".7z", ".7Z"];
